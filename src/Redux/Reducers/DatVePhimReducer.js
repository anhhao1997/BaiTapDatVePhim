...
export const DatVePhimReducer = (state = stateDefault, action) => {
  switch (action.type) {
    case "CHON_GHE": {
      let mangChonGheUP = [...state.danhSachGheChon];
      let gheDangChon = { ...action.ghe };
      let index = mangChonGheUP.findIndex(
        (ghe) => ghe.soGhe === gheDangChon.soGhe
      );

      if (index !== -1) {
        mangChonGheUP.splice(index, 1);
      } else {
        mangChonGheUP.push(gheDangChon);
      }

      state.danhSachGheChon = mangChonGheUP;
